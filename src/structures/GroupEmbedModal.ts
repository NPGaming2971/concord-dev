...
export class GroupEmbedModal extends EmbedBuilder {
	...
	public showName() {
		return this.setTitle(
			this.group.status === GroupStatusType.Private ? "Unknown" : this.group.getName()
		);
	}
	...
	public showOwner() {
		return this.addFields({
			name: "Owner",
			value:
				this.group.status !== GroupStatusType.Private
					? Formatters.userMention(this.group.ownerId)
					: "N/A",
			inline: true,
		});
	}
	...
	public showMemberCount() {
		return this.addFields({
			...
			value:
				this.group.status !== GroupStatusType.Private
					? `${this.group.channels.cache.size}/${this.group.channelLimit}`
					: "N/A",
			inline: true,
		});
	}
